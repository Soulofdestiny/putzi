...
function draw_departures(now, frame)
    ...
    for idx, dep in ipairs(departures) do
       -- print(dep.departure, dep.delay, dep.line, dep.destination, dep.platform)
       -- depinfo = dep.departure .. " \t " .. dep.delay .. " \t " .. dep.line .. " \t " .. dep.platform .. " \t " .. dep.destination
       -- print(depinfo)
        
        -- destination 
        font:write(x, y, dep.destination, 90, 1,1,1,1)
        
        -- departure time
        font:write(x+900, y, dep.departure, 90, 1,1,1,1)
        
        -- platform
        font:write(x+1400, y, dep.platform, 90, 1,1,1,1)
        
        -- delay
        delaynum = tonumber(string.match(dep.delay, "%d+"))
        
        if delaynum > 0 then
            font:write(x+1200, y, dep.delay, 90, 1,0,0,1)
        else
            font:write(x+1200, y, dep.delay, 90, 0,1,0,1)
        end

        -- line
            util.draw_correct(_G[dep.line], 10, y, 140, y+80, 0.9)

        y = y+110
        results = results - 1
        
        -- end when max_results were shown 
        if results == 0 then
            break
        end
...
function node.render()
    gl.clear(1, 0, 1, 1)
    schedule.draw()
    --font:write(120, 320, unixnow(), 100, 1,1,1,1)
end
